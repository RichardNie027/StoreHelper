...
public class Demo extends AppCompatActivity {
    ...
    private void requestAccessNetwork() {
        PermissionsUtil.requestPermission(getApplication(), new PermissionListener() {
            ...
            public void permissionGranted(@NonNull String[] permissions) {
                Toast.makeText(MyApplication.getInstance(), "访问网络", Toast.LENGTH_SHORT).show();
            }
    ...
    private void requestStorage() {
        PermissionsUtil.requestPermission(getApplication(), new PermissionListener() {
            @Override
            public void permissionGranted(@NonNull String[] permissions) {
                Toast.makeText(MyApplication.getInstance(), "读写文件", Toast.LENGTH_SHORT).show();
            }
    ...
    private void requestCemera() {
        PermissionsUtil.requestPermission(getApplication(), new PermissionListener() {
            @Override
            public void permissionGranted(@NonNull String[] permissions) {
                Toast.makeText(MyApplication.getInstance(), "访问摄像头", Toast.LENGTH_SHORT).show();
            }

            @Override
            public void permissionDenied(@NonNull String[] permissions) {
                Toast.makeText(MyApplication.getInstance(), "用户拒绝了访问摄像头", Toast.LENGTH_SHORT).show();
            }
        }, Manifest.permission.CAMERA);
    }

    private void requestReadContact() {
        PermissionsUtil.TipInfo tip = new PermissionsUtil.TipInfo("注意:", "需要查看通讯录", "禁止", "打开权限");
        PermissionsUtil.requestPermission(this, new PermissionListener() {
            @Override
            public void permissionGranted(@NonNull String[] permissions) {
                JSONArray arr = null;
                try {
                    arr = getContactInfo(Demo.this);
                    if (arr.length() == 0) {
                        Toast.makeText(MyApplication.getInstance(), "请确认通讯录不为空且有访问权限", Toast.LENGTH_SHORT).show();
                    } else {
                        Toast.makeText(MyApplication.getInstance(), arr.toString(), Toast.LENGTH_SHORT).show();
                    }
            ...
            @Override
            public void permissionDenied(@NonNull String[] permissions) {
                Toast.makeText(MyApplication.getInstance(), "用户拒绝了读取通讯录权限", Toast.LENGTH_SHORT).show();
            }
    ...
    private void requestSms() {
        ...
        PermissionsUtil.requestPermission(this, new PermissionListener() {
            @Override
            public void permissionGranted(@NonNull String[] permissions) {
                Toast.makeText(MyApplication.getInstance(), "访问消息", Toast.LENGTH_SHORT).show();
            }

            @Override
            public void permissionDenied(@NonNull String[] permissions) {
                Toast.makeText(MyApplication.getInstance(), "用户拒绝了读取消息权限", Toast.LENGTH_SHORT).show();
            }
    ...
    private void requestWriteContact() {
        PermissionsUtil.requestPermission(this, new PermissionListener() {
            @Override
            public void permissionGranted(@NonNull String[] permissions) {
                if (addContact("BillGates", "13901234567")) {
                    Toast.makeText(MyApplication.getInstance(), "成功添加联系人", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(MyApplication.getInstance(), "添加联系人失败", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void permissionDenied(@NonNull String[] permissions) {
                Toast.makeText(MyApplication.getInstance(), "用户拒绝了写通讯录", Toast.LENGTH_SHORT).show();
            }
    ...
    private boolean addContact(String name, String phoneNumber) {
        ...
        // 联系人的Email地址
        values.put(ContactsContract.CommonDataKinds.Email.DATA, "BillGates@hotmail.com");
        // 电子邮件的类型
